...
class BSBIIndex:
            ...
            with open(block_relative_path + '\\' + file) as f:
                ...
                for token in doc:
                    if not token.is_punct:
                        td_doc_pair.append((self.term_id_map[token.text], self.doc_id_map[block_dir_relative + file]))
    ...
    def merge(self, indices, merged_index):
        ...
        """
        for index in heapq.merge(*indices):
            merged_index.append(index[0], index[1])
...
if __name__ == "__main__":
    ...
    BSBI_EliasY_instance = BSBIIndex(data_dir = 'collection', \
                              postings_encoding = VBEPostings, \
                              output_dir = 'index')

    start = timeit.default_timer()
    BSBI_EliasY_instance.index() # memulai indexing!
    stop = timeit.default_timer()
    print('Indexing Time with Elias Gamma: ', stop - start)  
    ...
    stop = timeit.default_timer()
    print('Indexing Time with VB: ', stop - start)  